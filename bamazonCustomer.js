...
function viewProducts(connection) {
  ...
  var query = 'SELECT id, product_name, price, stock_quantity FROM bamazon_db.products';
  connection.query(query, function (err, res) {
    if (err) throw err;
    for (var i = 0; i < res.length; i++) {
      //display database contents 
      cl(' Product ID#: ' + res[i].id + ' | Product Name: ' +
...
function buyProduct(connection) {
  ...
  inquirer.prompt([{
    ...
    var query = 'SELECT product_name, price, stock_quantity FROM bamazon_db.products WHERE id = ' + answer.idNumber;
    connection.query(query, function (err, res) {
      if (err) {
        menu_options(connection);
        return;
